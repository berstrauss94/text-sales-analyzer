...
<head>
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        #tourCard .tour-text { font-size: 0.885rem; color: #cfd3dc; line-height: 1.6; margin-bottom: 14px; }
...
<div class="container">
    <div class="top-bar">
        <div>
            ...
            <p class="subtitle">Ventas y Bienes Raices &mdash; Analisis con Machine Learning <span id="versionBadge" onclick="toggleVersionInfo(event)" title="Toca para ver que trae esta actualizacion" style="font-size:0.7rem;font-weight:700;color:#4da3ff;background:rgba(77,163,255,0.12);padding:1px 7px;border-radius:8px;cursor:pointer;position:relative;">v21.1{% if username == 'Berna.Strauss' %} &middot; contrasenas con hash seguro (salt){% endif %}</span></p>
    ...
    <div class="input-section">
        ...
        <div class="highlight-define-row" id="highlightDefineRow">
            <div style="position:relative;display:inline-flex;align-items:center;gap:4px;">
                ...
                <span id="infoIconHelp" class="card-info-icon" onclick="event.stopPropagation()" style="position:static;flex:none;font-size:0.62rem;width:16px;height:16px;line-height:16px;text-align:center;">!</span>
            ...
            <div style="position:relative;display:inline-flex;align-items:center;gap:4px;margin-left:6px;">
                <button class="btn-highlight-define" id="btnEditDictionary" type="button" onclick="openDictionaryModal()">
                    &#128221; Editar diccionario
                </button>
                <span class="card-info-icon" onclick="event.stopPropagation()" style="position:static;flex:none;font-size:0.62rem;width:16px;height:16px;line-height:16px;text-align:center;">!</span>
        ...
        <div class="btn-row">
            ...
            <button id="btnClear" class="btn-secondary" onclick="clearAll()">Limpiar</button>
            {% if username in ['admin', 'Vanesa.Admin', 'Berna.Strauss', 'FedericoCeballos', 'MartinianoSosa'] %}
...
var UISound = (function() {
    ...
    // "Orchestra tuning" startup: from silence, several detuned voices around
    // A4 (440Hz) slowly converge to 440 with a very slow attack — no impact
    // transient. Warm, cinematic, concert-hall space. (per audio perspective)
    function orchestraTuning() {
        if (muted) return;
        var c = ac();
        if (!c || !master) return;
        try {
            ...
            var now = c.currentTime;
            var A4 = 440;
            // Voices spread across the harmonic series, each starting slightly
            // detuned (micro-dissonance) and gliding to its true pitch.
            var voices = [
                { mult: 0.5,  detune: -7 },   // A3 (warm low body)
                { mult: 1.0,  detune: +9 },   // A4 (reference)
                { mult: 1.0,  detune: -5 },
            ...
            voices.forEach(function(v, i) {
                ...
                lp.type = 'lowpass'; lp.frequency.value = 2400;   // spectral warmth
                osc.type = 'sine';
