...
def update_entry_text(username: str, entry_id: str, new_text_short: str,
                      new_text_full: str, users_dir: str = USERS_DIR) -> bool:
    """Update the text_short and text_full of an existing entry (preserves id/timestamp)."""
    if _is_pg_available():
        return _pg_update_entry_text(username, entry_id, new_text_short, new_text_full)
    return _json_update_entry_text(username, entry_id, new_text_short, new_text_full, users_dir)


def _pg_update_entry_text(username: str, entry_id: str, new_short: str, new_full: str) -> bool:
    conn = _get_pg_conn()
    if conn is None:
        return False
    try:
        with conn.cursor() as cur:
            cur.execute(
                "UPDATE analysis_history SET text_short = %s, text_full = %s "
                "WHERE username = %s AND id = %s",
                (new_short, new_full, username, entry_id)
            )
            conn.commit()
            return cur.rowcount > 0
    except Exception as exc:
        logger.error(f"PG update text error: {exc}")
        try:
            conn.rollback()
        except Exception:
            pass
...
def _json_update_entry_text(username: str, entry_id: str, new_short: str, new_full: str, users_dir: str) -> bool:
                ...
                for day_data in week_data.values():
                    ...
                    for entry in day_data.get("entries", []):
                        if entry.get("id") == entry_id:
                            entry["text"] = new_short
                            entry["text_full"] = new_full
                            updated = True
    if updated:
        path = _history_file(username, users_dir)
        # For subdirectory format, also handle that path
...
def _pg_add_entry(entry: dict, username: str) -> None:
    ...
    try:
        ...
        with conn.cursor() as cur:
            if audio_fname:
                # Insert new entry — ON CONFLICT DO NOTHING prevents exact ID duplicates
                cur.execute("""
                ...
                """, (
                    entry["id"], username, entry["timestamp"], entry["source"],
                    audio_fname, entry["text"], entry.get("text_full", entry["text"]),
                    entry["intent"], entry.get("intent_confidence", 0.0),
                    entry["sentiment"], entry.get("sentiment_confidence", 0.0),
