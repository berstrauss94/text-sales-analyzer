...
- [ ] 1. Crear diccionarios de categorías `_INDICADOR_CATEGORIAS` para los 6 indicadores planos en `src/components/commercial_analyzer.py`
  - [ ] 1.1 Crear el diccionario `_INDICADOR_CATEGORIAS: dict[str, dict[str, list[str]]]` después de `_PROSPECCION_CATEGORIAS`
  - [ ] 1.2 Definir subcategorías para `palabras_positivas`: "entusiasmo" (≥10 frases), "aprobacion" (≥10 frases), "satisfaccion" (≥10 frases)
  - [ ] 1.3 Definir subcategorías para `respuestas_afirmativas`: "confirmacion_directa" (≥10 frases), "acuerdo" (≥10 frases), "disposicion" (≥10 frases)
  - [ ] 1.4 Definir subcategorías para `indicios_cierre`: "accion_inmediata" (≥10 frases), "compromiso" (≥10 frases), "avance" (≥10 frases)
  - [ ] 1.5 Definir subcategorías para `escasez_comercial`: "disponibilidad" (≥10 frases), "urgencia_temporal" (≥10 frases), "limitacion" (≥10 frases)
  - [ ] 1.6 Definir subcategorías para `pedidos_referidos`: "solicitud_directa" (≥10 frases), "recomendacion" (≥10 frases), "red_contactos" (≥10 frases)
  - [ ] 1.7 Definir subcategorías para `objeciones`: "precio" (≥10 frases), "indecision" (≥10 frases), "postergacion" (≥10 frases)
  - [ ] 1.8 Verificar que TODAS las palabras de `_KEYWORDS` existente están incluidas en alguna subcategoría del indicador correspondiente
